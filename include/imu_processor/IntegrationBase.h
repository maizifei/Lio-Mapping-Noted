...
class IntegrationBase {
 ...
 public:
  ...
  void MidPointIntegration(double dt,
    ...
    result_delta_q = delta_q * Quaterniond(1, un_gyr(0) * dt / 2, un_gyr(1) * dt / 2, un_gyr(2) * dt / 2);  //姿态预积分测量值
    Vector3d un_acc_1 = result_delta_q * (acc1 - linearized_ba);
    Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);
    result_delta_p = delta_p + delta_v * dt + 0.5 * un_acc * dt * dt;  //位置预积分测量值
    result_delta_v = delta_v + un_acc * dt;  //速度预积分测量值
