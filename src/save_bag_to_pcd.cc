...
DEFINE_string(input_filename, "/home/belight/Dataset/20200114/test5_result.bag", "the name of input bag file");
DEFINE_string(output_filename, "/home/belight/Dataset/20200114/test5_result.pcd", "the name of output pcd file");
...
//  transform_to_init.linear() = tmp_quat.normalized().toRotationMatrix();
  ...
  LOG(INFO) << "merged: " << frame_count << "th frame";
  ++frame_count;
...
void LoadBag(const std::string &input_filename, const std::string &output_filename) {
  ...
  if (map_points.size() > 0) {
    ...
    LOG(INFO) << "saved as " << output_filename;
  } else {
