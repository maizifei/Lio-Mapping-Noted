...
//    DLOG(INFO) << "s: " << s;
    ...
    point.x -= s * transform_es.pos.x();  //这里是减，所以应该是转换到起始时刻
...
void Estimator::SetupRos(ros::NodeHandle &nh) {
  ...
  // imu_trans_.frame_id_ = "/world";
  // imu_trans_.child_frame_id_ = "/imu";

  // imu_trans_.setRotation(tf::Quaternion(0, 0, 0, 1));
  // imu_trans_.setOrigin(tf::Vector3(0, 0, 0));
...
//  }
  ...
  if (cir_buf_count_ != 0) {
    ...
    tmp_pre_integration_->push_back(dt, linear_acceleration, angular_velocity);  //预积分的迭代过程在这里进行
    ...
    Vector3d un_acc_0 = Rs_[j] * (acc_last_ - Bas_[j]) + g_vec_;  // g_vec_ = (0,0,-9.8)
...
//    DLOG(INFO) << imu_tt.transform;
  ...
  if (stage_flag_ == INITED) {
    ...
    // imu_trans_.setOrigin(tf::Vector3{Ps_.last().x(), Ps_.last().y(), Ps_.last().z()});
    // imu_trans_.setRotation(tf::Quaternion{quat.x(), quat.y(), quat.z(), quat.w()});
    // imu_trans_.stamp_ = header.stamp;
    // tf_broadcaster_est_.sendTransform(imu_trans_);
    // LOG(INFO) << "position imu predict: " << Ps_.last().transpose();
    // LOG(INFO) << "imu angular bias: " << Bgs_.last().transpose();
    // LOG(INFO) << "imu accel bias: " << Bas_.last().transpose();
  }
...
void Estimator::ProcessLaserOdom(const Transform &transform_in, const std_msgs::Header &header) {
  ...
  ROS_DEBUG(">>>>>>> new laser odom coming <<<<<<<");  //接收到新的雷达里程计数据

  ++laser_odom_recv_count_;
  LOG(INFO) << "laser_odom_recv_count = " << laser_odom_recv_count_;

  if (stage_flag_ != INITED
      && laser_odom_recv_count_ % estimator_config_.init_window_factor != 0) { /// better for initialization
    return;  //在初始化过程中每init_window_factor帧才进行下面的初始化计算
  }
  ...
  pre_integrations_.push(tmp_pre_integration_);  //来新的一帧激光雷达数据时，将上帧到此帧的预积分结果存入pre_integrations_

  // reset tmp_pre_integration_ 存完后，将tmp_pre_integration_重置，重新开始计算到下一帧雷达之间的预积分
  tmp_pre_integration_.reset();
  tmp_pre_integration_ = std::make_shared<IntegrationBase>(IntegrationBase(acc_last_,
                                                                           gyr_last_,
                                                                           Bas_[cir_buf_count_],
  ...
  if (estimator_config_.run_optimization) {
    switch (stage_flag_) {
      case NOT_INITED: {  //进行初始化
        ...
        if (cir_buf_count_ == estimator_config_.window_size) {  //buf中的帧数达到窗口宽度
          tic_toc_.Tic();

          if (!estimator_config_.imu_factor) {  //没有IMU信息
            init_result = true;
            ...
            if (extrinsic_stage_ == 2) {  //外参初始估计阶段，需要进行充分的转动运动
              ...
              LOG(INFO) << ">>>>>>> extrinsic calibration"
                        << (extrinsic_result ? " successful"
                                             : " failed")
                        << " <<<<<<<";
              if (extrinsic_result) {  //外参初始估计成功
                extrinsic_stage_ = 1;
                DLOG(INFO) << "change extrinsic stage to 1";
                LOG(INFO) << "extrinsic rotation: " << endl << transform_lb_.rot.normalized().toRotationMatrix();
              }
            }

            if (extrinsic_stage_ != 2 && (header.stamp.toSec() - initial_time_) > 0.1) {
              DLOG(INFO) << "EXTRINSIC STAGE: " << extrinsic_stage_;
              init_result = RunInitialization();  //进行算法初始化
      ...
      case INITED: {
        ...
        if (opt_point_coeff_map_.size() == estimator_config_.opt_window_size + 1) {
          ...
          if (estimator_config_.enable_deskew || estimator_config_.cutoff_deskew) {  //点云去畸变
            TicToc t_deskew;
            t_deskew.Tic();
            ...
            for (int i = int(imu_stampedtransforms.size()) - 1; i >= 0; --i) {
              ...
              if (time_e - imu_stampedtransforms[i].time >= 0.1) {  //找到当前scan起始时刻的imu变换
                break;
              }
            }
...
//            transform_body_es.pos = -0.1 * body_velocity.cast<float>();
            ...
            transform_es_ = transform_lb_ * transform_body_es * transform_lb_.inverse();  //一帧起始时刻到结束时刻的变换
            ...
            if (!estimator_config_.cutoff_deskew) {
              TransformToEnd(laser_cloud_surf_last_, transform_es_, 10);  // 将点云转换到scan起始时刻
...
//在初始化的过程中，此函数主要作用是解压点云数据CompactDataHandler()，调用PointMapping::Process()计算初始化窗口内的位姿transform_aft_mapped_，最后调用ProcessLaserOdom()
//在初始化完成后的正常运行过程中，此函数的主要作用只有解压点云数据，并调用ProcessLaserOdom()
void Estimator::ProcessCompactData(const sensor_msgs::PointCloud2ConstPtr &compact_data,
                                   const std_msgs::Header &header) {
  /// 1. process compact data
  PointMapping::CompactDataHandler(compact_data);  //更新laser_cloud_surf_last_  laser_cloud_corner_last_  full_cloud_
...
bool Estimator::RunInitialization() {
  ...
  // 检验初始化窗口内IMU数据的可观测性
  {
    ...
    for (size_t i = 0; i < estimator_config_.window_size;
      ...
      sum_g += tmp_g;
      // LOG(INFO) << "delta_v = " << laser_trans_j.second.pre_integration->delta_v_.transpose();
    }
    ...
    var = sqrt(var / (estimator_config_.window_size));  //所有帧加速度的标准差

    LOG(INFO) << "IMU variation: " << var;

    if (var < 0.25) {  //加速度标准差小于0.25,IMU不可观，运动激励不够
...
//  init_result = false;
  LOG(INFO) << "g_norm = " << g_vec_in_laser.norm();
//  Q_WI_ = R_WI_;
...
//  g_vec_ = Eigen::Vector3d(0.0, 0.0, -1.0) * g_norm_;
  ...
  LOG(WARNING) << "refined gravity:  " << g_vec_.transpose();

  if (!init_result) {
    LOG(WARNING) << "Imu initialization failed!";
    return false;
  } else {
...
//    DLOG(INFO) << "transform_to_local: " << transform_to_local;
  ...
  for (int i = 0; i < surf_points_size; i++) {
    ...
    PointAssociateToMap(point_ori, point_sel, transform_to_local);  //将原始点云中的特征点point_ori转换到局部地图的坐标系中得到point_sel
    // 为什么这里将原始点云中的point_ori转换到局部地图中用的变换transform_to_local是与建立局部地图时使用的变换是完全一样的？？？
    int num_neighbors = 5;
    ...
    if (point_search_sq_dis[num_neighbors - 1] < min_match_sq_dis_) {
      ...
      mat_X0 = mat_A0.colPivHouseholderQr().solve(mat_B0);  //求解 mat_A0*mat_X0 = mat_B0 方程  mat_X0即为5点构成的平面的法向量
      ...
      if (planeValid) {
        ...
        float s = 1 - 0.9f * fabs(pd2) / sqrt(CalcPointDistance(point_sel));  //鲁棒核函数
...
//      Eigen::Vector3f J_r = w.transpose() * RotationVectorJacobian(R_SO3, p);
      Eigen::Vector3f J_r = -w.transpose() * (local_transform.rot * SkewSymmetric(p));  //雅克比矩阵的前三列，误差对旋转量的偏导
      Eigen::Vector3f J_t = w.transpose();  //雅克比矩阵的后三列，误差对平移量的偏导

      float d2 = w.transpose() * (local_transform.rot * p + local_transform.pos) + coeff.intensity;  //参差项
    ...
    mat_X = matAtA.colPivHouseholderQr().solve(mat_AtB);  //求解方程 mat_At * mat_A * mat_X = mat_At * mat_B，得到优化后的位姿mat_X
...
//建立局部地图，将优化窗口内的所有帧的点云通过IMU积分得到的变换转换到pivot帧的坐标系下，得到局部地图
//其中最新帧的点云的转换是通过先进行scan to map的迭代匹配得到当前帧到局部地图的位姿变换，然后再用此变换将当前最新帧的点云转换到pivot帧坐标系
//转换完成后再通过scan to map的特征匹配得到进行优化计算需要的特征平面法向量、距离残差等
void Estimator::BuildLocalMap(vector<FeaturePerFrame> &feature_frames) {
...

  {
    ...
    if (!init_local_map_) {  //初始化局部地图，只会进行一次
    ...
    for (int i = 0; i < estimator_config_.window_size + 1; ++i) {
      ...
      // NOTE: exclude the latest one  最新帧的特征点不进行处理，即局部地图不包含最新帧的特征点云
      if (i != estimator_config_.window_size) {
        if (i == pivot_idx) {  // pivot帧直接加入局部地图
...
#endif
#else
        pcl::transformPointCloud(*(surf_stack_[i]), transformed_cloud_surf, transform_pivot_i);  // 特征点坐标变换
#ifdef USE_CORNER 
        pcl::transformPointCloud(*(corner_stack_[i]), transformed_cloud_corner, transform_pivot_i);
#endif
#endif
    ...
    down_size_filter_surf_.setInputCloud(local_surf_points_ptr_);
    down_size_filter_surf_.filter(*local_surf_points_filtered_ptr_);  // 降采样，得到局部地图平面特征点
  ...
  kdtree_surf_from_map->setInputCloud(local_surf_points_filtered_ptr_);  // 建立局部地图kdtree
...
//  loss_function = new ceres::HuberLoss(0.5);
  ...
  if (estimator_config_.update_laser_imu) {
    ...
    Quaterniond rot_pivot(Rs_pivot * transform_lb.rot.inverse());  // Lidar ----> World
    Eigen::Vector3d pos_pivot = Ps_pivot - rot_pivot * transform_lb.pos;

    Twist<double> transform_pivot = Twist<double>(rot_pivot, pos_pivot);  // Lidar ----> World
    // 根据IMU数据和外参计算优化窗口中所有IMU和雷达的位姿
    vector<Transform> imu_poses, lidar_poses;

    for (int i = 0; i < estimator_config_.opt_window_size + 1; ++i) {  //优化窗口中所有帧雷达的位姿 Lidar ----> World
...
//  P_last0 = Ps_.last();
  ...
  if (estimator_config_.marginalization_factor) {  //边缘化残差项
    if (last_marginalization_info) {
      // construct new marginlization_factor
      MarginalizationFactor *marginalization_factor = new MarginalizationFactor(last_marginalization_info);  //边缘化残差和Jacobian计算
      res_id_marg = problem.AddResidualBlock(marginalization_factor, NULL,
                                             last_marginalization_parameter_blocks);  //优化变量
  ...
  if (estimator_config_.imu_factor) {  
    ...
    for (int i = 0; i < estimator_config_.opt_window_size;
      ...
      ImuFactor *f = new ImuFactor(pre_integrations_[opt_j]);  //IMU预积分残差和Jacobian计算
//    {
...
//          res_ids_proj.push_back(res_id);
          ...
          PivotPointPlaneFactor *f = new PivotPointPlaneFactor(p_eigen,
                                                               coeff_eigen);  //Lidar残差和Jacobian计算
          ceres::internal::ResidualBlock *res_id =
              problem.AddResidualBlock(f,
                                       loss_function,
//                                     NULL,
...
//      if (cost > 1e3 || !convergence_flag_) {
...

    {
      ...
      if (!convergence_flag_ && !turn_off && ratio <= 2 && ratio != 0) {
        ...
        convergence_flag_ = true;  //满足收敛条件
      }

      if (!convergence_flag_) {  //如果不满足收敛条件
        ///<
        problem.SetParameterBlockConstant(para_ex_pose_);  //关闭外参估计和边缘化项
  ...
  ceres::Solve(options, &problem, &summary);  //优化方程解算
...
//    }
    ...
    //1.把之前的存的残差部分加进来
    //2.把与当前要margin掉帧所有相关的残差项都加进来，IMU,Lidar
    ...
    //5.margin结束，调整参数块在下一次window中对应的位置（往前移一格），注意这里是指针，后面slideWindow中会赋新值，这里只是提前占座
    ...
    if (last_marginalization_info) {
      ...
      ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(marginalization_factor, NULL,
                                                                     last_marginalization_parameter_blocks,  //优化变量
                                                                     drop_set);  //待marg的优化变量id

      marginalization_info->AddResidualBlockInfo(residual_block_info);
    }

    //2、将第p帧和第p+1帧间的IMU因子IMUFactor(pre_integrations[pivot_idx + 1])，添加到marginalization_info中
    if (estimator_config_.imu_factor) {
      int pivot_idx = estimator_config_.window_size - estimator_config_.opt_window_size;
      if (pre_integrations_[pivot_idx + 1]->sum_dt_ < 10.0) {
        ImuFactor *imu_factor = new ImuFactor(pre_integrations_[pivot_idx + 1]);
        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(imu_factor, NULL,
                                                                       vector<double *>{para_pose_[0],  //优化变量
                                                                                        para_speed_bias_[0],
                                                                                        para_pose_[1],
                                                                                        para_speed_bias_[1]},
                                                                       vector<int>{0, 1});  //待marg的优化变量id
        marginalization_info->AddResidualBlockInfo(residual_block_info);
      }
    }

    //3、将第一次观测为第0帧的所有特征信息，添加到marginalization_info中
    if (estimator_config_.point_distance_factor) {
      for (int i = 1; i < estimator_config_.opt_window_size + 1; ++i) {
        int opt_i = int(estimator_config_.window_size - estimator_config_.opt_window_size + i);
...
//    }
...

    {
      ...
      // LOG(INFO) << "extrinsic rotation after optimize: " << endl << transform_lb.rot.normalized().toRotationMatrix();
...
//Ps_、Rs_转变成para_pose_，Vs、Bas、Bgs转变成para_speed_bias_
    ...
    for (auto &measurement : measurements) {  // 遍历当前获得的所有帧雷达特征点云和对应的imu数据measurements
      ...
      for (auto &imu_msg : measurement.first) {
        ...
        if (imu_time <= laser_odom_time) {
          ...
          LOG(INFO) << "position imu predict: " << Vector3d(predict_odom_.pose.pose.position.x, predict_odom_.pose.pose.position.y, predict_odom_.pose.pose.position.z).transpose();
        }
      }
