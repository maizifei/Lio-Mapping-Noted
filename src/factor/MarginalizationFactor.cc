...
//调用Evaluate()计算各个部分的残差，参数块拷贝到parameter_block_data容器中
...
//多线程构造先验项舒尔补AX=b的结构，计算Jacobian和残差，，H是边缘化后的结果（根据parameter_block_idx里面的标记）
//使用Schur complement简化计算过程，First Esitimate Jacobian,在X0处线性化
void MarginalizationInfo::Marginalize() {
  ...
  */
  ...
  //FEJ First Estimate Jacobians，以后计算关于先验的误差和Jacobian都在边缘化的这个线性点展开
...
//先验残差，1.根据FEJ，固定线性化的点，更新这里的residuals和Jacobian
