...
void Run() {
  ...
  thread odom(&PointOdometry::Spin, &odometry);  //PointOdometry线程(纯LOAM激光里程计)，一直运行，直到退出ros

  thread measurement_manager(&Estimator::ProcessEstimation, &estimator);  //Estimator线程（紧耦合里程计），一直运行，直到退出ros
