...
void MapBuilder::Transform4DAssociateToMap() {
  ...
  double y_diff = origin_R0.x() - origin_R00.x();  //yaw的漂移

  //TODO
  Eigen::Matrix3f rot_diff = ypr2R(Eigen::Vector3f(y_diff, 0, 0));  //两次之间的位姿变化只保留yaw
...
//  PointMapping::SetupRos(nh, true);
  ...
  nh.param("skip_count", skip_count_, 1);
...
//  pub_diff_odometry_ = nh.advertise<nav_msgs::Odometry>("/laser_odom_to_last", 5);
  ...
  //订阅estimator节点发布的"/local_laser_odom"里程计话题，存入transform_sum_中
...
//  odom_aft_mapped_.twist.twist.linear.z = transform_bef_mapped_.pos.z();
  ...
  LOG(INFO) << "position after mapped: " << transform_aft_mapped_.pos.transpose();
...
//  }
  ...
  // 通过transform_tobe_mapped_将最新帧特征点转换到地图坐标系下
...
//根据transform_tobe_mapped_在cube地图中的位置滑动cube
  {
